...
function AnswerSurveyPopup(props) {
  ...
  var questionAmount =
    sortedSurveys.length > 0
      ? sortedSurveys[focusedSurveyIndex].questions.map((item) => false)
      : [];

  const [answeredQuestionArray, set_answeredQuestionArray] =
    useState(questionAmount);
      ...
      sortedSurveys[focusedSurveyIndex].questions.forEach((question, index) => {
        ...
        question.options.forEach((option, index) => {
          temp.push({
            ...
            answeredAmount: option.answers.length,
          });
        });
  ...
  function handleFormAction(event) {
    ...
    if (
      answeredQuestionArray.filter((c) => c).length !== questionAmount.length
    ) {
  ...
  function sendAnswers() {
    ...
    for (let index = 0; index < formData.length - 1; index++) {
      ...
      if (modifiedQuestion.type !== "metin") {
        let choiceIndex = modifiedQuestion.options.findIndex(
          (item) => item.name === answer
        );

        modifiedQuestion.options[choiceIndex].answers.push(
          ...
          <CardContent className="d-flex">
            <div className="create-survey d-flex flex-column p-4 rounded">
              ...
              >
                ...
                <DialogContent>
                  <DialogContentText>
                    Cevapladığınız Sorular{" "}
                    {answeredQuestionArray.filter((c) => c).length}/
                    {questionAmount.length}
                  </DialogContentText>
                </DialogContent>
                ...
                <DialogActions>
                  <Button
                    ...
                    onClick={() => set_finishSurveyDialog(false)}
                  >
                    Geri dön
                  </Button>
                  <Button
              ...
              <div className="d-flex flex-column " style={{ flexGrow: "1" }}>
                <div className="d-flex">
                  <Typography className="me-auto" variant="h3" gutterBottom>
                    {focusSurvey
                      ? sortedSurveys[focusedSurveyIndex].surveyTitle
                      : "Anket Cevapla"}
                  </Typography>
                  <div
                    style={{
                  ...
                  >
                    <Button
                      onClick={() => props.setAnswerSurvey(false)}
                ...
                {focusSurvey && (
                  <div className="d-flex justify-content-between">
                    <Button
                      style={{ fontSize: "medium", textTransform: "none" }}
                      onClick={() => {
                        set_focusSurvey(false);
                        set_answeredQuestionArray(questionAmount);
                        set_cevaplayanSicil("");
                    ...
                    >
                      {new Date() <
                      new Date(sortedSurveys[focusedSurveyIndex].expireDate)
                        ? `Anketin bitmesine son ${
                            calculateRemainingDays(
                              sortedSurveys[focusedSurveyIndex].expireDate
                            ) + 1
                          } gün.`
                        : "Anket bitmiştir."}
                    </Typography>
                ...
                <div
                  className="d-flex gap-3 flex-column mt-2"
                ...
                >
                  {sortedSurveys.length === 0 ? (
                    ...
                    sortedSurveys.map((survey, index) => {
                      ...
                      var remaningDays = calculateRemainingDays(
                        survey.expireDate
                      );
                      return (
                        <div key={index} className="d-flex flex-column rounded">
                          <Button
                            ...
                            disableElevation
                            className="align-items-start gap-1  d-flex flex-column"
                          >
                            <Typography
                              style={{ fontWeight: "200" }}
                    ...
                    <div className="d-flex flex-column" style={{ flex: "1" }}>
                      <div className="d-flex flex-wrap justify-content-between">
                        ...
                        >
                          Anketi oluşturan:{" "}
                          {sortedSurveys[focusedSurveyIndex].ownerId}
                        </Typography>
                        <Typography
                          style={{ fontWeight: "200" }}
                          variant="h6"
                      ...
                      {surveyResults === "cevapla" ? (
                        <form
                          id="answer-survey-form"
                          method="post"
                          onSubmit={handleFormAction}
                        >
                          {sortedSurveys[focusedSurveyIndex].questions.map(
                            (question, index) => (
                              <AnswerQuestionBody
                                handleAnsweredQuestionAmount={
                                  handleAnsweredQuestionAmount
                                }
                                key={index}
                                id={index}
                        ...
                        >
                          <SurveyResults
                            page={focusedQuestion - 1}
                            dataSet={dataSet[focusedQuestion - 1]}
                            question={
                              sortedSurveys[focusedSurveyIndex].questions[
                                focusedQuestion - 1
                              ]
                            }
                          >
                          ...
                          <div className="d-flex mt-3 justify-content-between">
                            <Pagination
                              ...
                              onChange={(event, value) =>
                                set_focusedQuestion(value)
                              }
                              page={focusedQuestion}
                              count={
                                sortedSurveys[focusedSurveyIndex].questions
                                  .length
                              }
                ...
                {focusSurvey && surveyResults === "cevapla" && (
                  <div className="d-flex justify-content-end mt-3 gap-3 align-items-baseline">
                    ...
                    <TextField
                      ...
                      value={cevaplayanSicil}
                      style={{ flex: "0.25" }}
                      inputProps={{ form: "answer-survey-form" }}
                      required
                      size="small"
                      label="Sicil"
                    ></TextField>
                    <Button
                      type="submit"
                      form="answer-survey-form"
                      className="rounded-pill"
                      variant="contained"
                      style={{ height: "2.7rem", textTransform: "none" }}
                    >
                      Cevapları Gönder
                    </Button>
                  </div>
