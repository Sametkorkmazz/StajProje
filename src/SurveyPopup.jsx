...
function SurveyPopup(props) {
  ...
  function handleDate() {
    ...
    var enteredDate = new Date(
      enteredDateString[0],
      parseInt(enteredDateString[1]) - 1,
      enteredDateString[2]
    );
  ...
  function newQuestion(type) {
    ...
    set_questionArray((prev) => [
      ...prev.map((item) => ({ ...item, expanded: false })),
      newQ,
    ]);
    document
      .getElementById("questions-body")
      .scrollTo({
        left: 0,
        top: document.getElementById("questions-body").scrollHeight,
        behavior: "smooth",
      });
  }
            ...
            <div className="create-survey d-flex flex-column p-4 rounded">
              <div className="d-flex justify-content-end">
                <Button
                  ...
                  onClick={() => props.setOpenSurveyCreation(false)}
              ...
              <div className="row py-2">
                <div className="col-6">
                  <TextField
                    ...
                    onInvalid={(event) =>
                      event.target.setCustomValidity("Bir başlık girin.")
                    }
                    inputProps={{
                ...
                <div className="col-12">
                  {questionArray.map((soru, index) => (
                    <Question
                      ...
                      changeAnswerLimit={changeAnswerLimit}
                      changeOptionsOrder={changeOptionsOrder}
              ...
              <div className="d-flex flex-column">
                {!addQuestionClicked ? (
                  <div className="col-12 mt-2 d-flex mb-auto">
                    <Button
                      colorpalette={"red"}
                      onClick={() => set_addQuestionClicked((prev) => !prev)}
                      variant="contained"
                      style={{ height: "2.7rem", textTransform: "none" }}
                    >
                      <AddIcon></AddIcon> Yeni soru
                    </Button>
                  </div>
                ) : (
                  <div className="col d-flex">
                    {" "}
                    <QuestionType
                      set_soruArray={newQuestion}
                      setSoruEkle={set_addQuestionClicked}
                    />{" "}
                  </div>
                )}
                <div className="row mt-4 d-flex flex-column justify-content-between">
                  <div className="col-12 mt-2 d-flex gap-3 flex-wrap">
                    <FormControlLabel
                      ...
                      control={
                        <Switch
                          size="small"
                          name="anonimAnswer"
                          value={surveyPreferences.anonimAnswer}
                          onClick={(event) => handleSurveyPreferences(event)}
                        />
                      }
                      label="Anonim sonuçlar"
                    />
                    ...
                    <FormControlLabel
                      ...
                      control={
                        <Switch
                          size="small"
                          name="anonimResult"
                          value={surveyPreferences.anonimResult}
                          onClick={(event) => handleSurveyPreferences(event)}
                        />
                      }
                      label="Anonim cevap"
                    />
                    <label className="align-self-center" htmlFor="expire-date">
                      Bitiş tarihi:
                    </label>
                    ...
                    <input
                      ...
                      value={surveyPreferences.expireDate}
                      onInvalid={(event) =>
                        event.target.setCustomValidity(
                          handleDate()
                            ? "Lütfen izin verilen aralıkta bir tarih giriniz."
                            : ""
                        )
                      }
                      onChange={(event) => {
                    ...
                    <form
                      ...
                      onSubmit={(event) => finishSurveyCreation(event)}
                    >
                      <TextField
                        onInvalid={(event) =>
                          event.target.setCustomValidity("Sicilinizi giriniz")
                        }
                        ...
                        variant="outlined"
                        size="small"
                        value={surveyPreferences.ownerId}
                        onChange={(event) => {
                          event.target.setCustomValidity(
                            event.target.value.trim().length === 0
                              ? "Sicilinizi giriniz"
