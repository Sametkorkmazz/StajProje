...
function Question(props) {
    ...
    function changeOptionsOrder(id, direction) {
        ...
        var tempArray = props.question.options.filter((option, index) => id !== index)
        var newArray = [...tempArray.slice(0, newIndex), option, ...tempArray.slice(newIndex)]
    ...
    function deleteOption(id) {
        var newOptions = props.question.options.filter((option, index) => index !== id)
        props.changeOptionsOrder(props.id, newOptions)
    ...
    return <Collapsible open={props.question.expanded}
        trigger={<div id={"" + props.id}  className='d-flex gap-1' onMouseOver={() => set_buttonsHovered(true)} onMouseOut={() => set_buttonsHovered(false)}>
            <Button variant='contained' className="mb-2 gap-2 justify-content-start" style={{ backgroundColor: "#684EB2", color: "white", textTransform: "none" }}
                ...
                {props.question.expanded ? <ExpandLessIcon fontSize='small'></ExpandLessIcon> : <ExpandMoreIcon fontSize='small'></ExpandMoreIcon>}
            ...
            <CardContent  >
                <div className="container" >
                    ...
                    <div className="d-flex">
                        ...
                        <TextField
                            onChange={(event) => props.changeQuestionName(props.id, event.target.value)}
                            id="standard"
                            value={props.question.questionName}
                            variant="standard" size="small" fullWidth InputLabelProps={{
                                shrink: true,
                                required: false,
                            }} >
                    ...
                    {props.question.multiChoice &&
                        <div className='mt-1 d-flex gap-3 align-items-baseline'>
                            ...
                            <Select
                                ...
                                size='small'
                                label=""
                                onChange={(event) => props.changeAnswerLimit(props.id, event.target.value, event.target.value === "Sınır yok" ?
                                    props.question.options.length : props.question.limit < 2 ? 2 : props.question.limit)
                                }
                                value={props.question.multiChoiceType}
                            >

                                <MenuItem value="Sınır yok">Sınır yok</MenuItem>
                                <MenuItem disabled={props.question.options.length < 2} value="Eşit">Eşit</MenuItem>
                                <MenuItem disabled={props.question.options.length < 2} value="En fazla">En fazla</MenuItem>




                            </Select>
                            {props.question.multiChoiceType !== "Sınır yok" && <Select
                                style={{ flex: 0.1 }}
                                size='small'
                                value={props.question.limit}
                                onChange={(event) => props.changeAnswerLimit(props.id, props.question.multiChoiceType, event.target.value)}


                            >
                                {props.question.options.slice(1).map((item, index) => <MenuItem value={index + 2}>{index + 2}</MenuItem>)}
                    ...
                    <div className='d-flex justify-content-end'>
                        {props.question.type === "seçenek" &&
                            <FormControlLabel control={<Switch checked={props.question.multiChoice} onClick={() => props.setMultiChoice(props.id)} />} label="Birden çok yanıt" />}
                        <FormControlLabel control={<Switch checked={props.question.required} onClick={() => props.setRequired(props.id)} />} label="Gerekli" />
                    </div>
                </div>
